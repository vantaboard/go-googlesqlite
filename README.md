...
**Shell env (host):** Source [`go-googlesql/scripts/go-googlesql-stack-bootstrap.sh`](https://github.com/vantaboard/go-googlesql/blob/main/scripts/go-googlesql-stack-bootstrap.sh) so **`CGO_LDFLAGS_ALLOW`**, **`CGO_LDFLAGS`**, and **`CGO_CXXFLAGS`** match [`Taskfile.yml`](https://github.com/vantaboard/go-googlesql/blob/main/Taskfile.yml) (sourcing `go-googlesql/.envrc` alone does not set linker allowlists). Then: **`go test -tags googlesql,googlesql_unified_prebuilt -p 1 -count=1 ./...`**, or use **`make test/prebuilt`**.
