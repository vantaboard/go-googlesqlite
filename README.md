...
**Tier B / prebuilt `go-googlesql` (optional):** If you build with **`-tags googlesql,googlesql_tier_b`**, you must supply **`libprotobuf_cgo.a`** in the **`go-googlesql`** tree (see [`docs/prebuilt-cgo.md`](https://github.com/vantaboard/go-googlesql/blob/main/docs/prebuilt-cgo.md) and release notes in [`CHANGELOG.md`](https://github.com/vantaboard/go-googlesql/blob/main/CHANGELOG.md)). Use the **same** tags and **`CGO_*`** flags as `go-googlesql`’s [`Makefile`](https://github.com/vantaboard/go-googlesql/blob/main/Makefile). The driver links one native stack—follow [`prebuilt-absl-overlap.md`](https://github.com/vantaboard/go-googlesql/blob/main/docs/prebuilt-absl-overlap.md) and do **not** combine unsupported tag mixes.
...
**Host-native builds and tests:** For incremental C++ compiles on the host, mirror [go-googlesql](https://github.com/vantaboard/go-googlesql#development): use **`CC="ccache clang"`** and **`CXX="ccache clang++"`**, point **`GOCACHE`**, **`GOMODCACHE`**, and **`CCACHE_DIR`** at the same tree (for example under **`GO_CACHE_ROOT`**), or run tests from **`go-googlesql`** with **`make -C ../go-googlesql local/test-root-unified`** (supported path: prebuilts + **`googlesql_unified_prebuilt`**) and **`TESTPKG=./...`** (or a narrower package path). Legacy **`local/test`** without unified prebuilt is **deprecated** in `go-googlesql` but may still match transitional CI—see [`link-only-cgo-migration.md`](https://github.com/vantaboard/go-googlesql/blob/main/docs/link-only-cgo-migration.md). On **Linux**, install **`mold`** and keep it on **`PATH`** so **`make local/build`** / **`local/test-root-unified`** in `go-googlesql` can pass **`CGO_LDFLAGS=-fuse-ld=mold`**; the **`go-googlesql:dev`** image already sets **`mold`** for Docker-based **`make test/linux`** here.
