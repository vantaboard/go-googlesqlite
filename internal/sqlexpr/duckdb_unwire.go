...
func DuckDBUnwireGooglesqlStringOperand(arg *SQLExpression) *SQLExpression {
	...
	header := NewFunctionExpression("try", NewFunctionExpression("lower", NewFunctionExpression("json_extract_string", payload, NewLiteralExpression(`'$.header'`))))
	body := NewFunctionExpression("try", NewFunctionExpression("json_extract_string", payload, NewLiteralExpression(`'$.body'`)))
...
func DuckDBGooglesqlWireArrayJSONForJsonEach(arg *SQLExpression) *SQLExpression {
	payload := duckDBWireGooglesqlUtf8Payload(arg)
	header := NewFunctionExpression("try", NewFunctionExpression("lower", NewFunctionExpression("json_extract_string", payload, NewLiteralExpression(`'$.header'`))))
	bodyStr := NewFunctionExpression("try", NewFunctionExpression("json_extract_string", payload, NewLiteralExpression(`'$.body'`)))
