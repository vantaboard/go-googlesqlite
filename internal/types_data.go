...
type ArrayScanData struct {
	...
	// ArrayExprElementType is the array element type from the resolved ArrayExpr (when the analyzer
	// provides ARRAY<...>). Used as a fallback when ElementColumnExpr is not struct-typed (e.g. wire
	// VARCHAR) so ARRAY<STRUCT<...>> UNNEST still projects gs__* struct columns.
	ArrayExprElementType types.Type      `json:"-"`
