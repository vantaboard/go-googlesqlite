...
var (
	...
	currentTimeFuncMap = map[string]struct{}{
		...
		"googlesqlengine_current_date":      struct{}{},
		"googlesqlengine_current_datetime":  struct{}{},
		"googlesqlengine_current_time":      struct{}{},
		"googlesqlengine_current_timestamp": struct{}{},
	}
)
...
func RegisterFunctions() error {
	...
	if err := sqlite.RegisterFunction("googlesqlengine_decode_array",
		&sqlite.FunctionImpl{
			Deterministic: true,
			NArgs:         -1,
	...
	if err := sqlite.RegisterFunction("googlesqlengine_group_by", &sqlite.FunctionImpl{
		Deterministic: true,
		NArgs:         -1,
		Scalar: func(ctx *sqlite.FunctionContext, args []driver.Value) (driver.Value, error) {
			decoded, err := DecodeValue(args[0])
		...
		return fmt.Errorf("failed to register function googlesqlengine_group_by: %w", err)
	}

	sqlite.MustRegisterCollationUtf8("googlesqlengine_collate", func(a, b string) int {
		va, _ := DecodeValue(a)
		vb, _ := DecodeValue(b)
		eq, _ := va.EQ(vb)
		if eq {
...
func setupNormalFuncMap(info *FuncInfo) error {
	normalFuncMap[info.Name] = &NameAndFunc{
		Name: fmt.Sprintf("googlesqlengine_%s", info.Name),
		Func: func(ctx *sqlite.FunctionContext, args []driver.Value) (driver.Value, error) {
	...
	normalFuncMap[safeName] = &NameAndFunc{
		Name: fmt.Sprintf("googlesqlengine_%s", safeName),
		Func: func(ctx *sqlite.FunctionContext, args []driver.Value) (driver.Value, error) {
			values, err := convertArgs(args)
			if err != nil {
				return nil, err
...
func setupWindowFuncMap(info *WindowFuncInfo) error {
	windowFuncMap[info.Name] = append(windowFuncMap[info.Name], &AggregateNameAndFunc{
		Name:          fmt.Sprintf("googlesqlengine_window_%s", info.Name),
		MakeAggregate: info.BindFunc(),
	})
	return nil
}
